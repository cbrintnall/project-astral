...
func execute(effect_ctx: EffectContext, exec_ctx: ExecutionContext):
  _reward_points(effect_ctx, _get_raw_point_count(effect_ctx))
  
func get_description(effect_ctx: EffectContext, exec_ctx: ExecutionContext) -> String:
  return "For each empty tile %s, gain %+.2f points ([color=#c69fa5]%s[/color])" % [ main_target.get_text(), points_per_empty, _get_total_points(effect_ctx, _get_raw_point_count(effect_ctx)) ]

func _get_raw_point_count(effect_ctx: EffectContext) -> int:
